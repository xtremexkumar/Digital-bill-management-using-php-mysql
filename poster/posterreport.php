...
$result = mysqli_query($con, "SELECT * FROM poster ORDER BY id DESC"); 
...
if(isset($_POST['search']))
{
	...
	$query = "SELECT * FROM poster WHERE CONCAT (`sno`,`Name`,`Contact`,`size`,`date`,`rate`,`totalsqr`,`subtotal`,`paid`,`pending`) LIKE '%".$valueToSearch."%'";
	
		
	
	 $query1 = "SELECT sum(a.rate) as rate1,sum(a.pending)as pending1,sum(a.paid)as paid1
	FROM poster a  WHERE CONCAT (`sno`,`Name`,`Contact`,`size`,`date`,`rate`,`totalsqr`,`subtotal`,`paid`,`pending`) LIKE '%".$valueToSearch."%'";
...
if(is_null($rate1 && $paid1 &&$pending1 )){
  ...
  echo '<script>alert("Record Not Found!");</script>';
	...
	$rowCount = mysqli_num_rows($result);
	if($rowCount<= 0){
  echo '<script>alert("Record Not Found!"); window.location = "posterreport.php";</script>';
  ...
  <?php 
    ...
    while($res = mysqli_fetch_array($result)){
       ...
       echo "<td>".$res['Contact']."</td>";
        ...
        echo "<td ><a href=\"posterupdate.php?id=$res[id]\">Edit</a> | <a href=\"delete.php?id=$res[id]\" onClick=\"return confirm('Are you sure you want to delete?')\">Delete</a></td>";
   echo "</tr>";	
