...
$result = mysqli_query($con, "SELECT * FROM expenses ORDER BY id DESC"); 
...
if(isset($_POST['search']))
{
	...
	$query = "SELECT * FROM expenses WHERE CONCAT (`name`,`quantity`,`rate`,`date`) LIKE '%".$valueToSearch."%'";
	
		
	
	 $query1 = "SELECT sum(a.rate) as rate1
	FROM expenses a  WHERE CONCAT (`name`,`quantity`,`rate`,`date`) LIKE '%".$valueToSearch."%'";
...
if(is_null($rate1)){
  ...
  echo '<script>alert("Record Not Found!");</script>';
	...
	$rowCount = mysqli_num_rows($result);
	if($rowCount<= 0){
  echo '<script>alert("Record Not Found!"); window.location = "expensesreport.php";</script>';
...
<form action="" method="post">
 ...
 <input   class="button" type="submit"  value="Go"name="search">
               ...
               <label for="dt">From Date: </label> <input name="from_date" type="date"/>  
                    <label for="dt">TO Date: </label> <input name="to_date" type="date"/>
<input type="text" name="dailyrate" id="dailyrate"value="<?php echo is_null($rate1)>0? "": $rate1;?>" size="7"/>					
  ...
  <?php 
    ...
    while($res = mysqli_fetch_array($result)){
      ...
      ;
        ...
        echo "<td ><a href=\"expensesupdate.php?id=$res[id]\">Edit</a> | <a href=\"expensesdelete.php?id=$res[id]\" onClick=\"return confirm('Are you sure you want to delete?')\">Delete</a></td>";
   echo "</tr>";	
