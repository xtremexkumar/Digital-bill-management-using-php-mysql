...
$result = mysqli_query($con, "SELECT * FROM visitingcard ORDER BY id DESC"); 
...
if(isset($_POST['search']))
{
	...
	$query = "SELECT * FROM visitingcard WHERE CONCAT (`sno`,`Name`,`Contact`,`size`,`date`,`Rate`,`paid`) LIKE '%".$valueToSearch."%'";
	
		
	
	 $query1 = "SELECT sum(a.rate) as rate1,sum(a.paid)as paid1
	FROM visitingcard a  WHERE CONCAT (`sno`,`Name`,`Contact`,`size`,`date`,`Rate`,`paid`) LIKE '%".$valueToSearch."%'";
...
if(is_null($rate1 && $paid1  )){
  ...
  echo '<script>alert("Record Not Found!");</script>';
	...
	$rowCount = mysqli_num_rows($result);
	if($rowCount<= 0){
 echo '<script>alert("Record Not Found!"); window.location = "vsreport.php";</script>';
  ...
  <?php 
    ...
    while($res = mysqli_fetch_array($result)){
       ...
       echo "<td>".$res['Contact']."</td>";
        ...
        echo "<td ><a href=\"vsupdate.php?id=$res[id]\">Edit</a> | <a href=\"delete.php?id=$res[id]\" onClick=\"return confirm('Are you sure you want to delete?')\">Delete</a></td>";
   echo "</tr>";	
