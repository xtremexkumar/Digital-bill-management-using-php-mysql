...
$result = mysqli_query($con, "SELECT * FROM star ORDER BY id DESC"); 
...
if(isset($_POST['search']))
{
	...
	$query = "SELECT * FROM star WHERE CONCAT (`sno`,`Name`,`Contact`,`size`,`date`,`rate`,`totalsqr`,`subtotal`,`paid`,`pending`) LIKE '%".$valueToSearch."%'";
	
		
	
	 $query1 = "SELECT sum(a.rate) as rate1,sum(a.pending)as pending1,sum(a.paid)as paid1
	FROM star a  WHERE CONCAT (`sno`,`Name`,`Contact`,`size`,`date`,`rate`,`totalsqr`,`subtotal`,`paid`,`pending`) LIKE '%".$valueToSearch."%'";
  ...
  <?php 
    ...
    while($res = mysqli_fetch_array($result)){
       ...
       echo "<td>".$res['Contact']."</td>";
        ...
        echo "<td ><a href=\"starupdate.php?id=$res[id]\">Edit</a> | <a href=\"stardelete.php?id=$res[id]\" onClick=\"return confirm('Are you sure you want to delete?')\">Delete</a></td>";
   echo "</tr>";	
