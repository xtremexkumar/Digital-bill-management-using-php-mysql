...
$id=$_REQUEST['id'];
$q=mysqli_query($connect,"select * from viniyl where id='$id'");
...
if(isset($update))
{
   mysqli_query($connect,"update viniyl set sno='$sno',Name='$Name',Contact='$Contact',Size='$Size',date='$date',Rate='$Rate',totalsqr='$totalsqr',subtotal='$subtotal',paid='$paid',pending='$pending',updateddate='" . $_REQUEST['tdate'] . "' WHERE id=$id");
		
	
	echo "Records updated";
	
	} 
echo "<script type='text/javascript'>alert('updated successfully!');window.location='viniylreport.php';</script>";
...
$result = mysqli_query($connect, "SELECT * FROM viniyl WHERE id=$id");
...
<div class="topnav">
  <a class="active" href="viniylsales.php">ViniylFlex</a>
 <a href="viniylpending.php">Pending</a>
  <a href="viniylreport.php">Report</a>
...
<h1  align="center"style="color:Green;">Rich Media ViniylFlex Update </h1>
<td >
<br/>
<br>
...
<td>		 
<input type="text" name="Size" autocomplete=off value="<?php echo $size;?>" size="7"id="Size" /> 
</td>
<td>		 
<input type="text" name="totalsqr" autocomplete=off value="<?php echo $totalsqr;?>" size="7"id="Totsqr" /> 
</td>
<td>		 
<input type="text" name="Rate" autocomplete=off value="<?php echo $rate;?>" size="7"id="Rate" onchange="sub1(document.getElementById('Rate').value);" /> 
</td>

<td>
<input type="text" autocomplete=off  size="7"name="subtotal"  readonly value="<?php echo $subtotal;?>" id="SubTotal" />
</td>
<td>
...
<td>
<input type="text" autocomplete=off  size="7"name="pending" readonly value="<?php echo $pending;?>" id="pending" />
</td>
<td>
